...
class FFChatInfo {
  ...
  final List<AllUsersRecord>? groupMembers;

  AllUsersRecord get currentUser => groupMembers!
      .where((user) => user.reference == currentUserReference)
      .first;
  Map<String, AllUsersRecord> get otherUsers => Map.fromEntries(
        groupMembers!
            .where((user) => user.reference != currentUserReference)
            .map((user) => MapEntry(user.reference.id, user)),
      );
  List<AllUsersRecord> get otherUsersList => otherUsers.values.toList();
...
class FFChatManager {
  ...
  Stream<FFChatInfo> getChatInfo({
    AllUsersRecord? otherUserRecord,
    DocumentReference? chatReference,
    ...
    return chatStream.asyncMap((chat) async {
      ...
      final groupMembers = await Future.wait(
        userRefs.map((ref) => AllUsersRecord.getDocument(ref).first),
      );
